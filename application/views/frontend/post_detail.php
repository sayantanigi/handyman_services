...
.Comment_Block { display: flex; flex-direction: row; align-items: flex-start; justify-content: flex-start; margin: 20px 0; }
.Comment_Block .Comment_Img { width: 10%; }
...
.Comment_Block .Comment_Data p:nth-child(1) {margin: 0; font-weight: 600; color: #000 !important; font-size: 16px;}
.Comment_Block .Comment_Data p:nth-child(2) {margin: 0; color: #000 !important; font-size: 14px; line-height: normal;}
                            ...
                            <div class="bid-dis">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-2">
                                        ...
                                        <a href="javascript:void(0)" style="background: linear-gradient(180deg, rgba(249, 80, 30, 1) 0%, rgba(252, 119, 33, 1) 100%); border: 0; border-radius: 100px; width: 100%; height: 45px; cursor: pointer; display:flex; align-items: center; justify-content: center;" onclick="postComment()">
                                            <span style="font-size: 15px; font-weight: 600; letter-spacing: 0; color: #fff;">Comment</span>
                                        </a>
                                        <?php } else { ?>
                                        <a href="<?= base_url()?>login" style="background: linear-gradient(180deg, rgba(249, 80, 30, 1) 0%, rgba(252, 119, 33, 1) 100%); border: 0; border-radius: 100px; width: 100%; height: 45px; cursor: pointer; display:flex; align-items: center; justify-content: center; ">
                                            <span style="font-size: 15px; font-weight: 600; letter-spacing: 0; color: #fff;">Comment</span>
                                        </a>
                                        <?php } ?>
                                    </div>
                                    ...
                                    <div class="col-12">
                                        ...
                                        ?>
                                        <div class="Comment_Block" style="background: aliceblue;padding: 15px;border-radius: 25px;">
...
function postComment() {
    if($('#comment').val() == "") {
		...
		setTimeout(function(){$("#err_comment").html("");},3000);
		$("#comment").css('border-color','red');
        setTimeout(function(){$("#comment").css('border-color','#80bdff');},3000);
		return false;
    } else {
...
function likepostjob() {
    ...
    $.ajax({
        ...
        type: "POST",
        data: { user_id: user_id, postjob_id: postjob_id },
        success: function (data) {
            location.reload();
        }
    })
...
function likeuserrply(id) {
    ...
    $.ajax({
        ...
        type: "POST",
        data: { user_id: user_id, postjob_id: postjob_id, comment_id: comment_id },
        success: function (data) {
            location.reload();
        }
    })
