    ...
    function subcategory_fetchdata($limit, $start, $title, $location,$days,$category_id,$subcategory_id,$duration,$pay_type,$charges,$post_id,$search_title,$search_location,$country,$state,$city) {
        ...
        if($data->num_rows() > 0) {
            foreach($data->result_array() as $row) {
                ...
                $output .= '<div class="emply-resume-list col-6"><div class="DataContainer">'.$profile_pic.'<div class="emply-resume-thumb">'.$jobimage.'</div><div class="emply-resume-info" style="min-height: 130px; max-height: 130px;"><h3><a style="width: 100% !important" href="'.base_url('workdetail/'.base64_encode($row['id'])).'" title="">'.$row['post_title'].'</a></h3><span>'.$get_category->category_name.'</span><span>'.$get_subcategory->sub_category_name.' </span><p><i class="la la-map-marker"></i>'.$row["location"].'</p></div>'.$bidBtn.'</div></div> ';
            }
        } else {
            $output .= '<div class="emply-resume-list"><div class="emply-resume-thumb"><h2>No Data Found</h2></div></div>';
        }
