...
class Welcome extends CI_Controller {
    ...
    public function update_postjob(){
        ...
        $post_title = $this->input->post('edit_post_title');
        $visibility = $this->input->post('edit_visibility');
        $location = $this->input->post('edit_location_guest');
        $latitude = $this->input->post('edit_search_lat_guest');
        $longitude = $this->input->post('edit_search_lon_guest');
        $category_id = $this->input->post('edit_category');

        // Handle file uploads
        $uploaded_files = [];
        if (!empty($_FILES['editpostjobPic']['name'])) {
            $filesCount = count($_FILES['editpostjobPic']['name']);
            for ($i = 0; $i < $filesCount; $i++) {
                $_FILES['file']['name'] = $_FILES['editpostjobPic']['name'][$i];
                $_FILES['file']['type'] = $_FILES['editpostjobPic']['type'][$i];
                $_FILES['file']['tmp_name'] = $_FILES['editpostjobPic']['tmp_name'][$i];
                $_FILES['file']['error'] = $_FILES['editpostjobPic']['error'][$i];
                $_FILES['file']['size'] = $_FILES['editpostjobPic']['size'][$i];

                $uploadPath = 'uploads/postjob/';
                $config['upload_path'] = $uploadPath;
                $config['allowed_types'] = 'jpg|jpeg|png|gif|webp|avi|mp4';
        ...
        if ($postjob_id) {
            ...
            $this->db->where('id', $postjob_id);
            $this->db->update('postjob', $data);
            ...
            if (!empty($uploaded_files)) {
                foreach ($uploaded_files as $file) {
                    ...
                    $this->db->insert('postjob_image', $fileData);
                }
            }
        } else {
            // Create new post
            ...
            $this->db->insert('postjob', $data);
            $postjob_id = $this->db->insert_id();
